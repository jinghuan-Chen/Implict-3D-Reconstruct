...
def get_points_from_vox(q, name_list, lock):
    ...
    for idx in range(name_num):
        ...
        with lock:
            voxel_model_256 = FutureVoxel256[name_list[idx]]
            voxel_model_256 = np.flip(np.transpose(voxel_model_256, (0,2,1)),2)
            voxel_model_256 = np.flip(np.transpose(voxel_model_256, (1,0,2)),2)
...
if __name__ == "__main__":
    ...
    global FutureVoxel256
    try:
        voxelhdf = h5py.File('/home/RS/CJH/chenjinghuan/3DFutureProject/data/3DFuture/ChairVoxelization/3DFutureChair256.h5', 'r')
    except:
        raise Exception("无法打开该H5文件！")
    FutureVoxel256 = voxelhdf["tensor"]
    FutureVoxel256_ = FutureVoxel256[10]

    class_name = "Chair256_00000000"
    print(class_name)
    ...
    name_num = FutureVoxel256.shape[0]
    num_of_process = 12
